...
void ADC_init()
{
	...
	ADC1->CFGR1 |= ADC_CFGR1_RES_1;//разрешение 8 бит
	ADC1->IER |= ADC_IER_ADRDYIE | ADC_IER_EOSMPIE;//прерывания
...
void ADC_IRQHandler(void){
	if((ADC1->ISR & ADC_ISR_EOSMP)){//событие по окончании записи
		res = ADC1->DR;
		ADC1->ISR|=ADC_ISR_EOSMP;

	}
	if ((ADC1->ISR & ADC_ISR_ADRDY)){//событие по готовности
